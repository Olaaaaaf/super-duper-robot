...
if (isset($_GET['export_csv']) && $_GET['export_csv'] == '1') {
    ...
    // Header CSV: sesuaikan dengan kolom yang akan diekspor
    // Menggunakan 'lampiran_izin_sakit' sesuai DB
    fputcsv($output, array('NIP', 'Nama Pegawai', 'Dinas', 'Tanggal', 'Jam Masuk', 'Jam Pulang', 'Status Kehadiran', 'Catatan', 'Lampiran'));

    // Query untuk export CSV
    // Menggunakan a.keterangan untuk status kehadiran, a.catatan, dan a.lampiran_izin_sakit
    ...
    if ($filter_dinas && $filter_dinas != 'Semua Dinas') {
...
// PERBAIKAN: Menggunakan a.catatan dan a.lampiran_izin_sakit AS lampiran
...
// Get all distinct dinas for the filter dropdown
$all_dinas = get_all_dinas($mysqli);
?>
                ...
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="filter_dinas" name="filter_dinas">
                            <option value="Semua Dinas" <?php echo ($filter_dinas == 'Semua Dinas') ? 'selected' : ''; ?>>Semua Dinas</option>
                            <?php foreach ($all_dinas as $dinas): ?>
                                <option value="<?php echo $dinas; ?>" <?php echo ($filter_dinas == $dinas) ? 'selected' : ''; ?>>
                                    <?php echo $dinas; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        ...
                        <?php foreach ($absensi_data as $absensi): ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    $status_class = '';
                                    switch ($absensi['status_kehadiran']) { // Menggunakan status_kehadiran karena sudah di-alias
                                        case 'Hadir':
                                        ...
                                        case 'Pending':
                                            ...
                                            break;
                                        case 'Disetujui':
                                            $status_class = 'badge bg-primary'; // Contoh warna baru
                                            break;
                                        case 'Ditolak':
                                            $status_class = 'badge bg-dark'; // Contoh warna baru
                                            break;
                                        default:
                                            $status_class = 'badge bg-secondary';
                                            break;
                                ...
                                <td>
                                    <?php if ($absensi['lampiran']): // Menggunakan 'lampiran' karena sudah di-alias di query ?>
            ...
            <nav aria-label="Page navigation example">
                <ul class="pagination justify-content-center">
                    <?php if ($current_page > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?php echo $current_page - 1; ?>&filter_date=<?php echo htmlspecialchars($filter_date); ?>&filter_nip=<?php echo htmlspecialchars($filter_nip); ?>&filter_dinas=<?php echo htmlspecialchars($filter_dinas); ?>&filter_status=<?php echo htmlspecialchars($filter_status); ?>">Previous</a>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if ($current_page < $total_pages): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?php echo $current_page + 1; ?>&filter_date=<?php echo htmlspecialchars($filter_date); ?>&filter_nip=<?php echo htmlspecialchars($filter_nip); ?>&filter_dinas=<?php echo htmlspecialchars($filter_dinas); ?>&filter_status=<?php echo htmlspecialchars($filter_status); ?>">Next</a>
                        </li>
                    <?php endif; ?>
                </ul>
            </nav>
